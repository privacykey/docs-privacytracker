...
If `docker compose up` stops before starting anything with `required variable AUDITOR_ADMIN_TOKEN is missing a value`, add a token to `.env` first (see [Installation → Docker](/installation)).
