...
A source checkout keeps its database at `./data/privacy.db`. Docker does too with the bind-mount file (see [Installation → Docker](/installation)); otherwise it keeps it in the `privacytracker-data` volume. So you can either:
