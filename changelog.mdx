...
- Rust core Phase 6, batch 6a: the Rust server is ready to face a network, ahead of the Docker image moving to it. `pt-core serve` takes `--host <ip>` (loopback by default), reads `PORT` as `next start` does when `--port` is absent, and refuses an option it does not know, where it used to ignore one: `--hostname 0.0.0.0` left the server on loopback, unreachable through a published port, while the in-container healthcheck still passed. With `--host`, what the security checks see as the bind host is where the server really listens, as `scripts/start-next.mjs` arranges for Node. A client that sends its headers too slowly is now cut off after 60 seconds, Node's own limit; the server used to wait indefinitely, because the HTTP stack it ran on never armed that timeout. Two lines Node prints at boot now appear from the Rust server too, word for word: the warning for an instance declared network-exposed with no admin token, and the error for a build missing its CSP hashes. Developer-facing until the Docker image runs this server.
- Rust core Phase 6, batch 5b: the desktop app on the Rust backend says what it is built from. `pnpm notices:rust` reads cargo's own dependency graph and writes the complete crate list (342 of them, with versions, licences and sources) plus a summary the app's Legal page renders; build and dev dependencies are excluded, since their code does not ship. Four files now travel inside the bundle under `third-party/`: `NOTICE`, `LICENSE`, `V8-LICENSE` (for the ports of V8's date parser and JSON error messages) and that crate list, and the release verifier refuses a bundle missing any of them or carrying an empty one. The Legal page gained a section naming the 37 crates chosen directly, the licence breakdown as the crates themselves spell it, and a pointer to the full list; its prose is translated like the rest of the page, while crate names, versions and licence identifiers stay in the upstream's own spelling. CI regenerates the list and fails on a difference, so a dependency change that is not disclosed cannot ship. Four negative controls fail exactly as predicted. Developer-facing only.
- Rust core Phase 6, batch 5a: the release workflow can build the desktop app on the Rust backend. **macOS desktop release** takes a `backend` input; `node` stays the default and is what every release still ships, while `rust` skips fetching, verifying and bundling Node, skips the staging keychain, stages the frontend instead of a standalone tarball, and signs with entitlements that grant none of the three the Node build needs (they exist for V8, and the webview's JIT runs in WebKit's own process). The bundle verifier gained a matching branch: it checks that nothing of Node ships, that the staged site is complete, that the binary carries none of those entitlements, and then drives the PACKAGED app over HTTP through a new hidden `--smoke-server <dir>` mode, running the same v0.1.2 upgrade, restore and restart rehearsal the Node build gets. Verified by building a bundle, signing it with the hardened runtime and running the verifier against it end to end, and by confirming the app's webview loads with an empty entitlements list. Three negative controls fail exactly as predicted. Tagged releases are unaffected until the cutover flips the input. Developer-facing only.
- Rust core Phase 6, batch 4b: a packaged desktop build on the Rust backend, and the database handed between the two. `scripts/stage-site.mjs` stages the frontend the Rust server serves into the app bundle's Resources: the prerendered pages with their payloads, the chunks, the CSP hashes and `public/`, about 9 MB where the Node path ships a ~200 MB tarball, and nothing that is server code. It refuses a build it cannot serve, a missing one, and any database file. `pnpm tauri:build:rust` (or `just tauri-build-rust`) builds with it through a config overlay, so default builds are untouched; a bundle built that way was run from outside the repository and served its pages from inside the app. A new handoff test (`just handoff`, and CI) has each backend seed and mutate its own data directory and then hands that directory, WAL and all, to the other, both ways round: 40 checks, which is what a rollback from a Rust release to a Node one rests on. Five negative controls fail exactly as predicted; two of them first disagreed, showing that two of the compared reads were empty and that a database with no devices changes under the receiving backend's own backfill. Both are now handled. Developer-facing only.
...
- **v0.2 upgrade requirements:** Docker deployments require
  ...
  to protect older Macs. See [release and recovery guidance](https://github.com/privacykey/privacytracker/blob/main/docs/RELEASING.md).
