...
<Tabs>
  <Tab title="Desktop app (macOS)">
    Signed and notarized for both Apple Silicon (`aarch64`) and Intel (`x86_64`). From v0.2 on it needs macOS 13.5 or later; v0.1.2 runs on macOS 11 and later.
    ...
    <Note>
      ...
      - The data folder stays where it is, and either build opens the database the other left behind, so the switch never stands in the way of [going back](/upgrading#rolling-back) to a release on Node.
      ...
      [Tauri & the backend](/develop/tauri) has the details.
    </Note>
  </Tab>
