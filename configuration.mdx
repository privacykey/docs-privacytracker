...
To turn policy changes on, override the flag to **on** under **Settings → Developer Options → Feature flags**. Upgrading keeps an override you already had. Without one, an upgraded install takes the default, so its policy events stop showing as changes to review.

<Warning>
  The **Privacy policy updates** checkbox under **Settings → Notifications** doesn't turn this flag on. Saving that section clears the override instead, and the checkbox unticks itself. The bell's list and badge also leave policy notifications out, even with the flag on, though the change still reaches the review panel and the webhook. See [Troubleshooting → Privacy policy updates checkbox unticks itself](/troubleshooting#privacy-policy-updates-checkbox-unticks-itself).
</Warning>
...
| `notification_webhook_frequency` | `immediate` / `daily_summary` / `weekly_summary` / `off` | `immediate` posts each privacy-policy change as it's raised. The summaries post the 50 most recent notifications raised since the previous summary, read or unread, from the 30-minute tick once a day or week has elapsed. |
| `notification_webhook_last_sent` | epoch ms | Written by the summary tick. Don't set it by hand. |

A privacy-policy change only reaches the webhook while [Privacy policy updates](#notifications) is on. Privacy-label changes and the bell's other notices, such as resumed runs, reach it through the daily and weekly summaries, not one at a time. With the default settings, `immediate` has nothing to post.
