...
privacytracker follows semver: the `<MAJOR>.<MINOR>.<PATCH>` of the *app* matches the version this docs site documents. The docs site is currently single-version (always reflects `main`), but the framework is in place to add per-version sub-sites when v1.2 ships with breaking config changes.
